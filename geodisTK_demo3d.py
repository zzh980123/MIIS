...
def demo_geodesic_distance3d():
    input_name = r"geodis_data/spleen_2.nii.gz"
    ...
    sitk.WriteImage(img_d1, "dataset_demo/image3d_dis1.nii.gz")

    img_d2 = sitk.GetImageFromArray(D2)
    img_d2.SetSpacing(spacing_raw)
    sitk.WriteImage(img_d2, "dataset_demo/image3d_dis2.nii.gz")

    img_d3 = sitk.GetImageFromArray(D3)
    img_d3.SetSpacing(spacing_raw)
    sitk.WriteImage(img_d3, "dataset_demo/image3d_dis3.nii.gz")

    I_sub = sitk.GetImageFromArray(I)
    I_sub.SetSpacing(spacing_raw)
    sitk.WriteImage(I_sub, "dataset_demo/image3d_sub.nii.gz")
